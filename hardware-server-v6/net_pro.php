...
//------------------------------------------------------------------------------------
	...
	function read_hw_state( $case_id ) {
		...
		$db = new db( $config );
		$res = $db->get_all( "SELECT dev_id, student_no, ins, ins_recv_t FROM devices_ctrl WHERE ctrl='$case_id'" );
		
		foreach( $res as $k => $v ) {
			$d_id = decode_dev_id( $v['dev_id'] );							// 设备在控制器内的id 
			if( $v['student_no']!='-1' && $v['ins']=='OPEN' && $v['ins_recv_t']<=time() )
				$st[$d_id-1] = 1;
			if( $v['student_no']!='-1' && $v['ins']=='CLOSE' && $v['ins_recv_t']>time() )		// 延时关闭指令，当前需要开启
				$st[$d_id-1] = 1;
		}
			...
			switch( $cur_state ) {
				...
				case '1':
					if( $dev_state==0 && $v['ins']=='OPEN' && $v['student_no']!='-1' ) {
						if( $v['open_t']==0 && time()>=$v['ins_recv_t'] )		// 如果是延时指令，不设置真实开启时间
							$data['open_t'] = time();
	...
	function check_db( $dev_ids ) {
		...
		foreach( $res as $v2 ) {			// 以ctrl为单位遍历设备
			...
			if( $v2['ins']=='OPEN' && time()>=$v2['ins_recv_t'] )				// 延时开启的设备，时间未到时，不设置开启
				$case[$v2['ctrl']][$d_id] = 1;
			
			// 延时关闭处理，未到关闭时间，设备需要开启
			if( $v2['ins']=='CLOSE' && time()<$v2['ins_recv_t'] )
				$case[$v2['ctrl']][$d_id] = 1;	
		...
		switch( $rec['student_no'] ) {
			...
			default:
				switch( $rec['ins'] ) {
					...
					case 'CLOSE':
						if( time()<$rec['ins_recv_t'] )			// 延时关闭指令，时间未到时，不处理
							break;
